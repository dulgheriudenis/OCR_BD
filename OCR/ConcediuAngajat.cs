            ...
            try
            {
                if (rezult == -1)
                {
                    ...
                    if (exist != "False")
                    {
                        ...
                        else throw new Exception("Perioada aleasa coincide cu un alt concediu inregistrat pentru angajatul : " + nume_si_prenume);
                ...
                else throw new Exception("Nu ati ales perioada corect !");
            }
            catch (Exception exc)
            {
        ...
        private void proceseaza_button_Click(object sender, EventArgs e)
        {
            try
            {
                if (nume_prenume_text_box.Text.ToString() != "")
                    ...
                    else throw new Exception("Angajatul nu exista in baza de date !");
                else throw new Exception("Introduceti numele si prenumele anagajatului !");
            }
            catch(Exception exc)
            {
                MessageBox.Show(exc.Message.ToString(), "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
